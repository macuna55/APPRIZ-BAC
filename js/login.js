...
function login(){
	  ...
	  var logAs = $('.loginBox input').eq(0).val().match(patFemail)[1];
		...
		$.post('http://'+IP+':8090/appriz/login_',{
...
function checkPreviusLogin(){
	...
	$.post('http://'+IP+':8090/appriz/getCurrentSession',{uuid:  typeof device !== 'undefined' ? device.uuid : "Browser" },function(data) {
...
$( document ).on("tapend","button.log",function(){
	...
	$.post('http://'+IP+':8090/appriz/logout', {"logId" : logId}, function(data){
		//console.log(data);
...
function resetPass(){
	...
	$.post('http://'+IP+':8090/appriz/setPassword_',{
		email			:   inputText,
